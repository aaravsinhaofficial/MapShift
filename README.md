...
This repository is the code artifact for the current MapShift release. It contains the environment generator, intervention operators, task samplers, benchmark health checks, baselines, study runner, raw-record schemas, reproduction commands, and paper table/figure renderers. Full raw records, health reports, rendered tables, figures, and manifests are generated by the commands below.

## Artifact Overview

This repository is a self-contained executable artifact. It regenerates benchmark instances, health checks, raw evaluation records, summary tables, rendered figures, and provenance manifests from versioned source code and configuration files. Generated instances are benchmark artifacts produced by the code, not a separately hosted static dataset. Output filenames and some config names retain the internal protocol id `cep` for compatibility; reviewer-facing prose uses the paper terminology CPE.
